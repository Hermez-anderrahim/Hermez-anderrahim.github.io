...
const SideBar = (props) => {
    ...
    setCountTODO(countTODO+1)
        const newObj = {id:links.length +1,text: `To-Do ${countTODO +2}`,show:true, showList:false}
    ...
    const showlinks =links.map((item)=>{
        if (item.show == true) {
            return (<ToDo data={item} deleteLink={deleteLink} key={item.id} switch={props.switchList} />)
